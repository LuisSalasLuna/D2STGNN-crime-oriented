...
class trainer():
    ...
    def train(self, input, real_val, **kwargs):
        n_nodes = 174 #------------------------- Redefinir segun conjunto
        self.model.train()
    ...
    def test(model, save_path_resume, device, dataloader, scaler, model_name, save=True, **kwargs):
        ...
        #Definir array
        crimes = []
        for _ in range(369): # año
            a = []
            for _ in range(n_nodes): #81 edit for crimes
                a.append([])
            crimes.append(a)
            ...
            if kwargs['dataset_name'] == 'PEMS04' or kwargs['dataset_name'] == 'PEMS08':  # traffic flow dataset follows mae metric used in ASTGNN.
                ...
                for d in range(len(pred)):
                    for p in range(len(pred[0])):
                        #print(pred[d][p].item())
                        crimes[d][p].append(pred[d][p].item())  
                #-----------------------------------------------------------
                log     = 'Evaluate best model on test data for horizon {:d}, Test MAE: {:.4f}, Test RMSE: {:.4f}, Test MAPE: {:.4f}'
        ...
        for i in range(len(crimes)):
            d = []
            for j in range(len(crimes[0])):
                #print(len(crimes[i][j]))
                #d.append(max(crimes[i][j])) #mean()?
                d.append(crimes[i][j][0])
            Dpred.append(d)

        maxi = []
        mini = []
        for i in range(len(Dpred)):
            maxi.append(max(Dpred[i]))
            mini.append(min(Dpred[i]))
            #print(max(Dpred[i]), min(Dpred[i]))
        ...
        if savePred:
            ...
            for i in range(n_nodes): # change too
                nueva_fila = { 'id': str(i), 'comp': float(Dpred[0][i])} # creamos un diccionario
                #dc = dc.append(nueva_fila, ignore_index=True)
                dc = pd.concat([dc, pd.DataFrame([nueva_fila])], ignore_index=True)
            dc.to_csv('./d2_174_p0.csv', index=False, encoding="utf-8") # Guardar1 (df a .cvs)
            ...
            for i in range(n_nodes): # change too
                nueva_fila = { 'id': str(i), 'comp': float(Dpred[1][i])} # creamos un diccionario
                #dc = dc.append(nueva_fila, ignore_index=True)
                dc = pd.concat([dc, pd.DataFrame([nueva_fila])], ignore_index=True)
            dc.to_csv('./d2_174_p1.csv', index=False, encoding="utf-8") # Guardar1 (df a .cvs)
            ...
            for i in range(n_nodes): # change too
                nueva_fila = { 'id': str(i), 'comp': float(Dpred[2][i])} # creamos un diccionario
                #dc = dc.append(nueva_fila, ignore_index=True)
                dc = pd.concat([dc, pd.DataFrame([nueva_fila])], ignore_index=True)
            dc.to_csv('./d2_174_p2.csv', index=False, encoding="utf-8") # Guardar1 (df a .cvs)
